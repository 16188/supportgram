...
export async function notifyCustomerIfOffline(conversation, business) {
  ...
  const htmlBody = `
    <p>${customerNameEsc}，您好：</p>
    <p>您在 <strong>${businessNameEsc}</strong> 的客服会话收到新回复。</p>
    <p>
      <a href="${resumeUrl}" style="display:inline-block;background-color:#1E8FD5;color:white;padding:10px 20px;text-decoration:none;border-radius:4px;font-weight:bold;">查看会话</a>
    </p>
    <p>也可以访问：${resumeUrl}</p>
  `;
