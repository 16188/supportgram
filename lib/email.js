// Brevo transactional email for offline customer notifications.
...
export async function notifyCustomerIfOffline(conversation, business) {
  // Skip silently if Brevo API key not configured.
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('email: Brevo API error:', response.status, errData?.message || '');
      return;
    }
