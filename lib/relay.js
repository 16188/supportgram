...
export async function startConversation({ business, name, email, pageUrl, firstMessage, ip, ipHash }) {
  ...
  const firstStored = await db.addMessage({
  ...
  try {
    ...
    try {
      const relayed = await sendMessage(business.bot_token, business.supergroup_id, esc(firstMessage), {
        message_thread_id: topicId,
      });
      await db.setMessageTelegramId(firstStored.id, relayed.message_id);
    } catch (err) {
...
export async function customerMessage(conversation, business, body) {
  const stored = await db.addMessage({
    conversation_id: conversation.id,
    direction: 'in',
  ...
  if (conversation.topic_id) {
    try {
      const relayed = await sendMessage(business.bot_token, business.supergroup_id, esc(body), {
        message_thread_id: conversation.topic_id,
      });
      await db.setMessageTelegramId(stored.id, relayed.message_id);
    } catch (err) {
...
export async function customerMedia(conversation, business, media) {
  ...
  if (conversation.topic_id) {
    try {
      const relayed = await sendMedia(business.bot_token, business.supergroup_id, media, {
        message_thread_id: conversation.topic_id,
        caption: media.name,
...
export function telegramReactionEmojis(reactions = []) {
...
export async function handleTelegramUpdate(business, update) {
  ...
  if (reaction) {
    if (String(reaction.chat?.id) !== String(business.supergroup_id)) return null;
    const actorId = reaction.user?.id;
    if (!actorId || reaction.user?.is_bot) return null;
    const agents = await db.getActiveAgents(business.id);
    if (!agents.some((agent) => Number(agent.tg_user_id) === Number(actorId))) return null;
    await db.setMessageReactionsByBusiness(
      business.id,
      reaction.message_id,
      actorId,
      telegramReactionEmojis(reaction.new_reaction)
    );
    return null;
  }
  ...
  if (edited) {
    ...
    const updated = await db.editAgentMessageByBusiness(business.id, msg.message_id, body);
    if (!updated) {
      console.warn(`handleTelegramUpdate: edited message ${msg.message_id} was not found`);
    }
    return null;
  }
  ...
  const repliedMessage = repliedMessageId
    ? await db.getMessageByTelegramId(conversation.id, repliedMessageId)
    : null;
  ...
  try {
    await db.addMessage({
      ...
      reply_quote: repliedMessage && typeof msg.quote?.text === 'string' ? msg.quote.text : null,
      media_type: media?.type,
      media_path: media?.path,
      media_name: media?.name,
      media_mime: media?.mime,
