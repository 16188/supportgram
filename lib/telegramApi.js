...
export function deleteMessage(botToken, chat_id, message_id) {
  return tg(botToken, 'deleteMessage', { chat_id, message_id });
}

export function setWebhook(botToken, url, secret_token) {
  return tg(botToken, 'setWebhook', {
    url,
    secret_token,
...
export const AGENT_COMMANDS = [
  ...
  { command: 'undo', description: '回复一条客服消息并撤回网页和 Telegram 消息' },
  { command: 'block', description: '拉黑当前访客，阻止继续发送和新建会话' },
  { command: 'unblock', description: '解除当前访客的黑名单限制' },
  { command: 'delete', description: '永久删除会话、VPS 文件和 Telegram 话题' },
];
